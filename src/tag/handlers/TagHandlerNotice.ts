...
export class TagHandlerNotice extends TagHandler {
    tagName(): string {
        return 'notice';
    };
    ...
    constructor() {
        super();
        // 拼音兼容
        TagHandlerNotice.typeMap['gonglue'] = TagHandlerNotice.typeMap['gonglve'];
        TagHandlerNotice.typeGroupMap['gonglue'] = TagHandlerNotice.typeGroupMap['gonglve'];
    }

    encodeToHtml(tagLabel: string, arg: string, content: string, forEditor: boolean = false): string | false {
        const attr = TagHandlerNotice.typeMap[arg];
        if (!attr) {
            return `[notice=${arg}]`;
        }
    ...
    decodeFromHtml(element: Element, resolveFun: (node: Nodes, forEditor: boolean) => string, forEditor: boolean): string | false {
        ...
        const group = TagHandlerNotice.typeGroupMap[type];
